...
Legend::~Legend()
{
    ...
    glDeleteVertexArrays(1, &m_vaoLegend);
    glDeleteBuffers(1, &m_vboLegend);
    glDeleteBuffers(1, &m_eboLegend);

    glDeleteTextures(1, &m_textureLocation);

    qDebug() << "In Legend destructor";
}
...
void Legend::initializeGL() {
    ...
    // Generate buffers
    glGenVertexArrays(1, &m_vaoLegend);
    glGenBuffers(1, &m_vboLegend);
    glGenBuffers(1, &m_eboLegend);

    glGenTextures(1, &m_textureLocation);
    loadInitialColorMap(); // Load single black color colorMap by default.

    // Buffer data
    glBindVertexArray(m_vaoLegend);

    glBindBuffer(GL_ARRAY_BUFFER, m_vboLegend);
    ...
    // Buffer indices
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_eboLegend);
    ...
    glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<GLvoid*>(2 * sizeof(GLfloat)));
}
